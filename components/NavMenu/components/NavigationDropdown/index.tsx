...
export const NavigationDropdown: FC<INavigationDropdownProps> = ({ onClose }): JSX.Element => {
  ...
  return (
    <nav
      ref={ref}
      className='absolute z-50 right-2 mx-1 top-20 overflow-hidden bg-white shadow-lg'
    >
            ...
            {Pages.map((page, index) => (
              <li className='grow' key={index}>
                <Link
                  ...
                  title={page.title}
                  className='py-3 px-3 max-sm:px-2 border-b hover:bg-[#E8E9DD] flex text-sm leading-[18px] tracking-[0.01em] transition-all font-bold no-underline'
                >
                  {page.title}
                </Link>
              </li>
